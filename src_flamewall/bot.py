...
from flamewall_obs import FlamewallObsBuilder, BOOST_LOCATIONS
...
class Flamewall(BaseAgent):
    ...
    def initialize_agent(self):
        ...
        self.obs_builder = FlamewallObsBuilder(field_info=self.field_info)
    ...
    def get_output(self, packet: GameTickPacket) -> SimpleControllerState:
        ...
        if len(self.game_state.players) > self.index:
            ...
            Flamewall._shared_state['positions'][self.index] = my_pos
            Flamewall._shared_state['boost_status'][self.index] = player.boost_amount
            ...
            if len(teammates) > 1:
                ...
                current_chaser = Flamewall._shared_state.get('ball_chaser')
                if current_chaser is None or current_chaser not in Flamewall._shared_state['positions']:
            ...
            is_chaser = (Flamewall._shared_state.get('ball_chaser') == self.index)
            Flamewall._shared_state['assignments'][self.index] = 'attacker' if is_chaser else 'support'
